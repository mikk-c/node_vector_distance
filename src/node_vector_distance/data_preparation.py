...
class AttrGraph(object):
   def __init__(self, G, df, edge_attr_order = None, workflow = "gpu"):
      G, df, self.nodemap = self._extract_lcc(G, df)
      ...
      if workflow == "gpu":
         self.data = self._make_tensor(G, df, edge_attr_order, edge_weights)
      else:
         self.data = self._make_graph(G, df, edge_attr_order, edge_weights)
   ...
   def _extract_lcc(G, df):
      ...
      G, nodemap = self._convert_to_int_node_ids(G)
      df = self._consolidate_node_attributes(df, nodemap)
      return G, df, nodemap